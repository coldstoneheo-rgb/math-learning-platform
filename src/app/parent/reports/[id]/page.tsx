    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        {analysis.growthPredictions && analysis.growthPredictions.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">📈 성장 예측</h3>
            <div className="grid md:grid-cols-3 gap-4">
              {analysis.growthPredictions.map((prediction, index) => (
                <div key={index} className="border border-gray-200 rounded-lg p-4 text-center">
                  <div className="text-sm text-gray-500 mb-1">{prediction.timeframe} 후</div>
                  <div className="text-3xl font-bold text-indigo-600">{prediction.predictedScore}점</div>
                  <div className="text-xs text-gray-400 mt-1">신뢰도 {prediction.confidenceLevel}%</div>
                </div>
              ))}
            </div>
          </div>
